...
int uuid_symm_crypt_start(void * sub_proc,void * para)
{
	int ret;
	void * recv_msg;
	int i;
...
int proc_crypt_message(void * sub_proc,void * message)
{
	...
	RECORD(MESSAGE,UUID_RECORD)* uuid_record;
	...
	ret=message_remove_expand(message,TYPE_PAIR(MESSAGE,UUID_RECORD),&msg_expand);
	if(ret<0)
		return ret;
	uuid_record=msg_expand->expand;

	bind_blob_size=sm4_context_crypt(blob,&bind_blob,blob_size,uuid_record->uuid);
	if(bind_blob_size<0)
...
int proc_uncrypt_message(void * sub_proc,void * message)
{
	...
	blob_size=sm4_context_decrypt(bind_blob,&blob,bind_blob_size,uuid_record->uuid);
	if(blob_size<0)
		return blob_size;
